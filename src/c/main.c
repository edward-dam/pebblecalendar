...
static void load_options() {
  ...
  if (persist_exists(hour_setting)) {
    char buffer[5];
    persist_read_string(hour_setting, buffer, sizeof(buffer));
...
static void year_tick_handler(struct tm *tick_time, TimeUnits units_changed) {
  update_year();
}
...
static void month_tick_handler(struct tm *tick_time, TimeUnits units_changed) {
  update_month();
}
...
static void canvas_update_proc(Layer *layer, GContext *ctx) {
  ...
  GRect day_square  = GRect(cx-65,cy-55,85, 25);
  GRect batt_header = GRect(cx+30,cy-50,10, 15);
  GRect batt_square = GRect(cx+35,cy-55,30, 25);
  GRect hour_square = GRect(cx-65,cy-25,60, 50);
  GRect mins_square = GRect(cx +5,cy-25,60, 50);
...
static void main_window_load(Window *window) {
  ...
  // time layers
  hour_layer = text_layer_create(GRect(cx-63,cy-31,mx,my));
  mins_layer = text_layer_create(GRect(cx +7,cy-31,mx,my));
  ...
  text_layer_set_font(mins_layer, fonts_get_system_font(FONT_KEY_ROBOTO_BOLD_SUBSET_49));
  layer_add_child(window_layer, text_layer_get_layer(hour_layer));
  layer_add_child(window_layer, text_layer_get_layer(mins_layer));

  // date layers
  day_layer   = text_layer_create(GRect(cx-64,cy-60,85,my));
...
static void init() {
  ...
  update_hour();
  update_mins();
  tick_timer_service_subscribe(YEAR_UNIT,   year_tick_handler);
  tick_timer_service_subscribe(MONTH_UNIT,  month_tick_handler);
